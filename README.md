...
- 코드 수정
  ...
  - 코루틴 사용.
- 경고 소리 감소.
- 연결 확인 대기시간 감소: 10초 -> 3초.
- 클라이언트: 서버와의 연결을 실시간으로 확인함.

### 2.0.0
- 공통: 예외로 인해 유저의 연결이 끊길 경우 소리 추가.
- 클라이언트: 접속이 끊겨도 매크로가 종료되지 않도록 수정.
- 서버: 매 10초마다 연결된 클라이언트에게 상태 신호 전송. 10초 이상 응답이 없으면 해당 클라이언트 연결 종료.
